...
**API:** `POST /api/v2/campaigns`

## Notes (important)

- **Email bodies must be HTML**. Plain text with `\n` newlines does **not** render as line breaks in Instantly.
- **Use HTML tags for formatting**, e.g. `<br/>` for line breaks and `<p>...</p>` / `<div>...</div>` for spacing/structure.
- **Use `{{sendingAccountFirstName}}` to sign off emails** with the sender's first name. This dynamically inserts the first name of the sending account.
...
```typescript
{
  ...
  sequences?: Array<{
    steps: Array<{
      ...
      variants: Array<{
        subject: string;
        body: string; // IMPORTANT: must be HTML (use <br/> or <p> for line breaks). "\n" will not render as a new line.
...
### Example: HTML body with proper structure

Use `<div>` tags for paragraphs and `<div><br /></div>` for spacing between paragraphs:
          ...
          variants: [
            {
              ...
              body: "<div>Hi {{first_name}},</div><div><br /></div><div>Quick observation from talking to Clay agency founders:</div><div>Every client engagement, your team rebuilds 70-80% of the same enrichment workflows. Same columns, same logic, different workspace. That's hours of billable work burned on repetitive setup.</div><div><br /></div><div>We built a way to templatize full workflows- not just prompts. One agency cut client onboarding from 6 hours to 30 minutes.</div><div><br /></div><div>That's margin you're leaving on the table with every new client.</div><div><br /></div><div>Worth a 15 min look?</div><div><br /></div><div>Best,</div><div>{{sendingAccountFirstName}}</div>",
            },
          ],
        },
      ],
...
- `<div>...</div>` — Wraps each line/paragraph
- `<div><br /></div>` — Creates a blank line between paragraphs
- Variables like `{{first_name}}` go inside the div content
- Use `{{sendingAccountFirstName}}` in the sign-off to dynamically insert the sender's first name
